...
export const TaskColoumn = ({ projectId }) => {
  ...
  console.log("Inside TaskCl", projectId);
  ...
  useEffect(() => {
    const fetchTasks = async () => {
      try {
        ...
        console.log("Inside Fetch Task in Task Coloumn Data", data);
        setTasks(data);
  ...
  //     const data = await response.json();
  //     console.log("Inside Task Coloumn", data);
  //     console.log("Inside Task Coloumn", data.title);
  //     if (!response.ok) {
  ...
  //       `http://localhost:4000/tasks/${data._id}`,
  //       {
  //         method: "PUT",
  //         headers: {
  //           "Content-Type": "application/json",
  ...
  return (
    <div className="p-4">
      {console.log("Inside TaskColoumn", tasks)}
